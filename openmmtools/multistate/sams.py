...
class SAMSSampler(multistate.MultiStateSampler):
    ...
    def _restore_sampler_from_reporter(self, reporter):
        ...
        logger.debug('Restored round trips: {}'.format(self._round_trips))
    ...
    def _update_stage(self):
        ...
        if (self.update_stages == 'two-stage') and (self._stage == 0):
            ...
            if (bool(advance) and all(v == True for v in advance.values())) or ((self._t0 > 0) and (self._iteration > self._t0)):
                ...
                logger.debug('{} round-trips'.format(self._round_trips))
                logger.debug('criteria satisfied: {}'.format(advance))
                self._stage = 1 # asymptotically optimal
                self._t0 = self._iteration - 1

        elif (self._stage == 1):
            #check change in weights every 1000 sams iterations
            if (self._iteration % 200) == 0:
                if (self.logZ_saved is not None):
                    criteria = np.abs(self._logZ[1:] - self.logZ_saved[1:]) < 0.1
                    logger.debug('window criteria is: {}'.format(criteria))
                    if np.all(criteria):
                        # change window and initiate another sams simulation
                        self._change_window = True
                    self.logZ_saved = self._logZ.copy()
                else:
                    self.logZ_saved = self._logZ.copy()
